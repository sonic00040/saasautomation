  ...
  "inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        ...
        gradient:
          "border-transparent bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:from-blue-700 hover:to-purple-700 shadow-sm",
        dot: "border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 shadow-sm relative pl-6",
      },
      size: {
...
function Badge({ className, variant, size, ...props }: BadgeProps & VariantProps<typeof badgeVariants>) {
  return (
    <div className={cn(badgeVariants({ variant, size }), className)} {...props}>
      {variant === 'dot' && (
        <div className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-current rounded-full opacity-60" />
      )}
      {props.children}
    </div>
  )
